...
from math import pi
import matplotlib.lines as mlines
big_params = genfromtxt(bigparams.txt, skip_header = 1)

#big_line = 22
#big_CO2 = 26
#big_NH3 = 23
treeoutput = open("treeoutput.txt", "r")
big = genfromtxt("waterbig4.txt", skip_header=3)
big_mass = big[:,4]
big_frac = (big_params[0,:]*.1)
CO2_frac_big = (big_params[0,:]*.029)
NH3_frac_big = (big_params[0,:]*.05)
...
small_params = genfromtxt(smallparams.txt, skip_header = 1)
#small_line = 228
#small_CO2 = 260
#small_NH3 = 229
small = genfromtxt("watersmall4.txt", skip_header=2, invalid_raise = False)
small_mass = small[:,4]
small_frac = (small_mass[:]*.1)
CO2_frac_small = (small_mass[:]*.029)
NH3_frac_small = (small_mass[:]*.05)
if small_params[0,:] > .32:
	small_frac = 0
if small_params[0,:] > .61:
	CO2_frac_small = 0
if small_params[0,:] > .61:
	NH3_frac_small = 0
